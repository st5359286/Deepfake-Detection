...
function resetProgress() {
  if (analyzingTitle) analyzingTitle.textContent = "Uploading...";
  if (progressWrapper) progressWrapper.classList.remove("hidden");
  if (progressBar) {
    progressBar.style.width = "0%";
    progressBar.textContent = "0%";
  }
  if (analyzingLoader) analyzingLoader.classList.add("hidden");
...
async function startAnalysis(file) {
  ...
  progressBar.textContent = "0%";
  if (analyzingTitle)
    analyzingTitle.textContent = "Analyzing Media Signature...";
  ...
  try {
    if (!crypto || !crypto.subtle) {
      throw new Error(
        "Secure Context required for analysis (HTTPS or localhost).",
      );
    }
    ...
    if (file.size < 5 * 1024 * 1024) {
      // Limit to 5MB for localStorage safety
      ...
      console.log(
        "File too large for local preview storage, skipping preview.",
      );
...
async function handleGenerateSummary() {
  ...
  try {
    ...
    summaryText.textContent =
      error.message ||
      "Apologies, the summary could not be generated at this time.";
    summaryPanel.classList.remove("hidden");
  } finally {
